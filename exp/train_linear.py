...
def main():
    batch_size, num_epochs, lr, k = 128, 100, 0.1, 5
    loss = nn.BCEWithLogitsLoss()
    train_dataSet = Get_data_linear()
    device = dlf.devices()[0]

    model = nn.Sequential(nn.Linear(202, 256), nn.ReLU(), nn.Linear(256, 8))
    ...
    PATH = "../data/exp_data/linear_parameters.pth"  # 你想要保存的路径和文件名

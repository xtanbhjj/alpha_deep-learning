...
def validate(model, device, loss, val_loader, epoch, num_epochs):
    ...
    with torch.no_grad():
        for images, masks in tqdm(val_loader, desc=f"Epoch {epoch+1}/{num_epochs} (Validation)"):
            ...
            outputs = model(images)
            #resized_outputs = F.interpolate(outputs, size=(256, 1600), mode='bilinear', align_corners=False)
            l = loss(outputs, masks) # 使用 Focal Loss + Dice Loss
...
class CombinedLoss(nn.Module):
    def __init__(self, alpha=[2.0, 2.0, 1.0, 1.5], gamma=2.0, smooth_dice=1.0, dice_weight=1, bce_weight=1, focal_weight=1):
        super().__init__()
    ...
    def forward(self, pred_logits, target_masks):
        bce = self.bce_loss(pred_logits, target_masks)
